...
var _ = Describe("sensor self-registration", func() {
	It("emits NodeType Create and Node Create events on startup", func() {
		...
		master := sensor.TestOnlyMasterEvents(srv)
		...
		Expect(master[0].ResourceId).To(Equal(sensor.TestOnlyNodeTypeID()))
		...
		nodeID := sensor.TestOnlyNodeID(srv)
	...
	It("generates a different nodeID on each startup", func() {
		...
		Expect(err).NotTo(HaveOccurred())
		id1 := sensor.TestOnlyNodeID(srv1)
		_ = srv1.Close()

		srv2, err := sensor.New("127.0.0.1:0", nil, log)
		Expect(err).NotTo(HaveOccurred())
		id2 := sensor.TestOnlyNodeID(srv2)
		_ = srv2.Close()
